        ...
        <div class="bg-white dark:bg-gray-800 border rounded-lg p-5 shadow">
            <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input name="name" value="{{ old('name', $product->name ?? '') }}" required
                            class="mt-1 w-full border rounded px-3 py-2 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-black">
                    </div>
                    ...
                    <div>
                        ...
                        <input name="selling_price" type="number" step="0.01"
                            value="{{ old('selling_price', isset($product->selling_price) ? number_format($product->selling_price, 2, '.', '') : '') }}"
                            class="mt-1 w-full border rounded px-3 py-2">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-200">Stok</label>
                        <input name="stock" type="number" value="{{ old('stock', $product->stock ?? 0) }}"
                            class="mt-1 w-full border rounded px-3 py-2 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-black">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="4"
                            class="mt-1 w-full border rounded px-3 py-2 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-black">{{ old('description', $product->description ?? '') }}</textarea>
                    </div>
                </div>
                ...
                <div class="mt-4 border-t pt-4">
                    ...
                    <div class="space-y-3">
                        @foreach (\App\Models\Attribute::with('options')->get() as $attr)
                            <div>
                                <div class="text-sm font-semibold text-gray-600 dark:text-gray-300">{{ $attr->name }}
                                </div>
                                @if ($attr->options->count())
                                    <select name="attributes[{{ $attr->id }}]"
                                        class="mt-1 w-full border rounded px-3 py-2 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                        <option value="">-- Pilih --</option>
                                        @foreach ($attr->options as $opt)
                                            <option value="{{ $opt->id }}" @selected(old("attributes.{$attr->id}", optional($product)->attributeValue($attr->id)) == $opt->id)>
                                                {{ $opt->value }}</option>
                                        @endforeach
                                    </select>
                                @else
                                    <input name="attributes[{{ $attr->id }}]"
                                        value="{{ old("attributes.{$attr->id}", optional($product)->attributeValue($attr->id)) }}"
                                        class="mt-1 w-full border rounded px-3 py-2 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                @endif
                            </div>
                        @endforeach
                ...
                <div class="mt-6 flex items-center justify-between">
                    <div class="text-sm text-gray-500">Terakhir diubah:
        ...
        <div class="relative p-4 w-full max-w-md h-full md:h-auto mx-auto mt-20">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
                <button type="button" class="absolute top-3 right-2.5 text-gray-400"
                    data-modal-toggle="deleteModal">×</button>
                <div class="p-6">
                    <h3 class="mb-4 text-lg font-medium text-gray-900 dark:text-white">Hapus Produk</h3>
                    <p class="text-sm text-gray-500 mb-4">Anda yakin ingin menghapus produk ini? Tindakan ini tidak
                        dapat dibatalkan.</p>
