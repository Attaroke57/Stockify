<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
        ...
        <input name="price" value="{{ old('price', $product->price ?? '') }}" type="number" class="w-full border rounded px-3 py-2">
    </div>

    <div>
        <label class="block text-sm">Stok</label>
        <input name="stock" value="{{ old('stock', $product->stock ?? 0) }}" type="number" class="w-full border rounded px-3 py-2">
    </div>
    ...
    <div>
        ...
        <div class="space-y-1">
            ...
            <div class="text-sm">
                ...
                @else
                    <input name="attributes[{{ $attr->id }}]" value="{{ old("attributes.$attr->id") }}" class="w-full border rounded px-2 py-1" placeholder="Masukkan nilai">
                @endif
            </div>
            @endforeach
        </div>
