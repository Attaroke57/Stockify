            ...
            <div class="glass-card rounded-3xl shadow-2xl p-6 mb-6 fade-in">
                <form method="GET" class="flex flex-wrap gap-3">
                    ...
                    <input type="date" name="to" value="{{ request('to') }}"
                        class="input-field flex-1 min-w-fit border-2 border-gray-200 rounded-xl px-4 py-3 font-medium focus:outline-none">
                    <select name="type"
                        class="input-field flex-1 min-w-fit border-2 border-gray-200 rounded-xl px-4 py-3 font-medium focus:outline-none appearance-none bg-white">
                        <option value="">Semua Tipe</option>
                        <option value="in" @selected(request('type') == 'in')>📥 Masuk</option>
                        <option value="out" @selected(request('type') == 'out')>📤 Keluar</option>
                    </select>
                    <button type="submit"
                        class="btn-gradient px-6 py-3 text-white rounded-xl font-semibold flex items-center">
                    ...
                    <a href="{{ route('reports.transactions.export', request()->query()) }}"
                        class="btn-light px-6 py-3 text-gray-700 rounded-xl font-semibold flex items-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <tr class="table-row">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            <div
                                                class="w-10 h-10 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center text-white font-bold mr-3">
                                                {{ substr($t->product->name ?? 'P', 0, 1) }}
                                            </div>
                                            <span
                                                class="font-semibold text-gray-800">{{ $t->product->name ?? '-' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <span
                                            class="row-badge px-3 py-1 rounded-lg text-sm font-bold {{ $t->type === 'in' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-500 font-semibold text-lg">Tidak ada transaksi ditemukan
                                            </p>
                                            <p class="text-gray-400 text-sm mt-1">Coba ubah filter atau tambah transaksi
                                                baru</p>
                                        </div>
                                    </td>
                                </tr>
